---
layout: post
title: Hyper-V Virtual Devices Overview
description: Some device driver issues related Linux Hyper-V guest VM, especially IDE controller issues
...
This article tries to give a simple overview about the virtual platform provided by Hyper-V, and how Linux OS works insides in a guest VM.
...
Before looking into the detailed information defined in the specification, there are some key concepts need to be clarified.
...
- [Para Virtualization](https://en.wikipedia.org/wiki/Paravirtualization)
  ...
  The mechanism that allows guest OS to cooperate with hypervisor is called by a terminology: **hypercall**.

At beginning, Para Virtualization usually provides the better performance than Full Virtualization on x86 platform.
...
the Full Virtualization VM could perform better than Para Virtualization VM nowadays.
...
- Emulated Devices
  ...
  The devices are emulated by Hyper-V and acting as the real hardware. That means drivers in guest OS just drive the hardware devices as same as they do against physical devices.
  A legacy OS without any Hyper-V knowledge wouldn't work if emulated devices were not supported.

  The emulated devices could be probed or detected by low level tools or firmware which are based on well known hardware/bus standards. For example, If a hardware is emulated as a PCI device,
...
- Synthetic Devices
  ...
  and only can be driven by virtual device drivers in guest OS.

  There are no ways to probe the Synthetic Devices without a virtual device drivers in a guest OS.
...
- Direct Pass-through Devices
  ...
  support could be assigned to the VM. Otherwise, a buggy hardware or drivers could result a memory corruption against other VMs or hypervisor.
...
Microsoft called this effort with **LIS(Linux Integration Services) Drivers**. The latest LIS driver code is always available from Linux kernel mainline.
...
The LIS backporting project is [hosted as an separate open source project from Github](https://github.com/LIS/lis-next).
...
- Intel PATA/SATA driver (ata_piix)
  ...
  The driver is not recommended on Hyper-V. When this driver is loaded, the IDE disks are named by `/dev/sd[a|b|c|d]`.
  The reason ata_piix uses the different naming here is, libata library is built under Linux SCSI stack, which eventually cooperates with SCSI mid layer and SD driver.
...
Hyper-V for windows server only supports booting from IDE disks instead of booting from SCSI disks or external SAN storages.

The major reason of this limitation is, Hyper-V doesn't implement an emulated SCSI controller for its guest VMs.
That means, when VMs get booted from BIOS loading phase, the BIOS has no way to probe SCSI disks as they are not emulated devices. Instead, they are synthetic devices.
...
However, [Hyper-V for virtual server support emulated SCSI controller](https://blogs.technet.microsoft.com/jhoward/2007/10/04/boot-from-scsi-in-virtual-server-vs-boot-from-ide-in-windows-server-virtualization-hyper-v/),
...
- First of all, legacy ide-generic and ata_piix drivers could be controlled by different kernel config.

  Linux distribution usually disable legacy IDE drivers and use ata_piix driver to drive the emulated IDE devices in Hyper-V VM.
...
PXE boot is the similar case with boot disk. Hyper-V only support one type of emulated devices for NIC controller.
