...
- cond_resched 或 yield

  Linux 内核的 Kernel Preemption 没有打开的话，除了系统调用，中断，异常返回用户空间时发生 Preemption，`cond_resched` 和 `yield` 是唯一的方式防止内核滥用 CPU 了。
...
- 禁止抢占上下文结束时。
  ...
  调用 `preempt_enable`，`preempt_check_resched` 和 `preempt_schedule`。稍后会详细介绍相关的实现。

#### 5.2.1 preempt_count

TBD

#### 5.2.2 preempt_disable 和 preempt_enable
...
如 2.3.2.2 小节所述，preempt_disable 和 preempt_enable 函数也被嵌入到很多内核函数的实现里，例如各种锁的进入和退出函数。
